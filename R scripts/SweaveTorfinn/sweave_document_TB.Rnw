...
\noindent
\Sexpr{out}. (see Figure ~ \ref{funnelplot}) 
...
print(xtable(mytable2, 
             ...
             align = "rccccc|ccccc",  digits=3, display=c("s", rep("fg", 10))), caption.placement = "top", size ="footnotesize", scalebox = 0.9, sanitize.text = force)
@
\bigskip
...
# If function for the leave1out analysis.  
 ...
 if (rma.RE$pval < 0.05) {
  ...
  out2 = paste("The leave-one out analysis shows that the effect size of the meta-analysis is not significant anymore if", 
               ...
               paste(which(sens.RE$pval > 0.05), collapse=", "),
               
               (if (sum(sens.RE$pval > 0.05)>1) {"are"} 
                else {"is"}), 
               "left out. If there are many left-out studies yielding non-significance relative to the total number of studies in the meta-analysis, it is a sign of a fragile analysis") 
  ...
  if (length(which(sens.RE$pval < 0.05) > 0)) { 
  out2 = paste("The leave-one out analysis shows that significance is yielded by leaving out", 
               if (sum(sens.RE$pval > 0.05)>1) {"studies"} 
               else {"study"}, 
               paste(which(sens.RE$pval < 0.05), collapse=","),". 
