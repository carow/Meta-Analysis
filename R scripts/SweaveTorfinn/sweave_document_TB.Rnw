...
This is an automated appendix which displays important diagnostics to spot possible shortcomings of a meta analysis, or for example causes for heterogenity across studies. The document is intended both to print relevant information about the findings in the meta analysis, as well as guiding the reader through important factors to look at when assessing a meta analysis. 
