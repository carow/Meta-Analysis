...
text(0, rma.RE$k+2, "Standardised mean difference", cex=.68, font=2)
...
matrix.RE = data.frame(t(c("Effect Size"=rma.RE$b, "SE of Effect Size"=rma.RE$se, "CI (lb)"= rma.RE$ci.lb, "CI(ub)" = rma.RE$ci.ub, "P(ES)" = rma.RE$pval, "Q" = rma.RE$QE, "P(Q)" = rma.RE$QEp, "I^2" = rma.RE$I2, "Egger" = egger.RE$zval, "P(Egger)" = egger.RE$pval, "FSN" = fsn.RE$fsnum)))
rownames(matrix.RE) <- "Value"
require(xtable)
mytable = matrix.RE
@

<<xtable1, results = tex, echo=FALSE>>=
print(xtable(mytable, caption = paste("Results of the meta-analysis. Includes the the egger's test and the fails-safe number for publication bias testing.")),caption.placement = "top", size ="footnotesize", scalebox = 0.9)
@
\bigskip
...
rownames(matrix.RE.meta) <- "Africa"
require(xtable)
...
\captionsetup{width=0.6\textwidth}
\centering
<<label=Galbraith, echo=FALSE, fig=TRUE>>=
#Standardized residuals plot does not have a ready-made function, and must be coded. Code used is from the plot.rma.uni.
gp = par(mar = c(4,3,1,0))
radial(rma.RE, zlab = "Standardized residuals", xlab = "Inverse standard deviation", main = "Galbraith's radial plot", cex=1)
par(gp)
@
...
\captionsetup{width=0.6\textwidth}
\centering
<<label=qqnorm, echo=FALSE, fig=TRUE>>=
#Standardized residuals plot does not have a ready-made function, and must be coded. Code used is from the plot.rma.uni.
...
\captionsetup{width=0.6\textwidth}
\centering
<<label=diagnostics, echo=FALSE, fig=TRUE>>=
#Standardized residuals plot does not have a ready-made function, and must be coded. Code used is from the plot.rma.uni.
...
baujat(rma.RE, main = "Baujat heterogenity diagnostic plot", mgp=c(2,1,0))
par(mar = c(4,3,1,0))


radial(rma.RE, zlab = "Standardized residuals", xlab = "Inverse standard deviation", main = "Galbraith's radial plot", cex=1, mgp=c(2,1,0))
