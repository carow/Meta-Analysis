...
qqnorm(rma.RE, label = "all", mgp=c(2,1,0), pch = NA_integer_) #how to get study names color red?
...
#title("Galbraith's radial plot", line = -6)
...
<<Diagnostics, echo=FALSE, dev="pdf">>=
#Standardized residuals plot does not have a ready-made function, and must be coded. Code used is from the plot.rma.uni.
par (mfrow= c(2,2), mar = c(4,4,1,2))
par(mar = c(4,4,1,1))
z = rstandard(rma.RE)$z
