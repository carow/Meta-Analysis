...
The Q-value (see Table~\ref{xtable1}) is the weighted sum of squares for the effect sizes of the studies in the meta-analysis. It is a standardized measure of the variation of the effect sizes across the studies analyzed.\\

The p-value for Q (see P(Q) in Table \ref{xtable1}) is the likelihood of the null-hypothesis that all studies share a common effect size. A p-value < 0.05 means we can reject the null-hypothesis, indicating that the studies in the analysis do not share a common effect size. A p-value > 0.05 means we can retain the null-hypothesis. This should, however, not be taken as an evidence that the effect sizes are consistent. It can also be due to low statistical power (Borenstein et. al. 2009, p. 112).\\ 
...
%Eggers test, the slope will indicate the size and direction of the effect size and the intercept will be 0. The deviation of the intercept from zero provides and index of degree of asymmetry.Tests for a relationship between observed outcomes(sample size, SE...) and effect size. If a relationship is present, implies asymmetry. 
...
\Sexpr{out} (see Figure~\ref{funnelplot}) as the P(Egger) is \Sexpr{signif(egger.RE$pval, 3)}.\\
...
{\Sexpr{out}} as it is \Sexpr{fsn.RE$fsnum}. The meta-analysis is robust against publication bias according to Rosenthal, when the fail-safe number is greater than (5*(number of studies) +10). 
...
width = max(rma.RE$yi + qnorm(0.05/2, lower.tail = FALSE) * sqrt(rma.RE$vi)) - min(rma.RE$yi - qnorm(0.05/2, lower.tail = FALSE) * sqrt(rma.RE$vi))

forest.rma(rma.RE, annotate = TRUE, cex = 0.7, showweight = TRUE, addcred = TRUE, xlab = "")#, xlim = c((-(width))+(0.2*width), (width)+(0.5*width))
text(0, rma.RE$k+2, "Standardized Mean Difference", cex = .68, font = 2)
text( (-(width)+(0.2*width))/2, rma.RE$k+2, "Study",    pos = 4, cex = .68, font = 2)
text(width + 2, rma.RE$k+2, "Weights   Effect sizes [95% CI]",  pos = 2, cex = .68, font = 2)
par (mar = c(5,4,4,2))
@
...
print(xtable(mytable2, 
             ...
             align = "rccccc|ccccc",  digits=3, display=c("s", rep("fg", 10)), , label = "xtable2"), caption.placement = "top", size ="footnotesize", scalebox = 0.9, sanitize.text = force)
@
\bigskip
...
When publication bias is absent, the plot should have a symmetrical shape of a funnel around the mean effect size. This is because the precision of the estimation of effect size should increase with sample size (smaller standard error). If many studies lie outside of this funnel shaped space, then the studies used might need to be reviewed and checked for..
