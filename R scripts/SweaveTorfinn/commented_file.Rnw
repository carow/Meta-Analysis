...
\caption{Funnel plot of random effects model displaying possible publication bias. The standardized mean difference (SMD) is in our case the effect size (ES).  The true ES is displayed by the solid verical line.}
\end{figure}
