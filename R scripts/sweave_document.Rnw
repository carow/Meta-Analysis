...
\captionsetup{width=0.6\textwidth}
<<label=tab:two,echo=FALSE,results=tex>>=

mytable=summary(bestmodel)

print(xtable(mytable, caption = "Parameter estimates of the regression fit to predict tree volume depending on girth in cherry trees. Note that a quadratic term has been added to the model structure", label = "tab:two",
digits = c( 0, 2,2, 2, 2)),
