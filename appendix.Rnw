...
The $I^2$ indicates the overall heterogenity on a relative scale (percent). For this analysis the $I^2$ is \Sexpr{signif(rma.RE$I2,3)}. The rule of thumb is that an $I^2$ around 25 \% indicates low heterogenity, 50 \% indicates moderate heterogenity and 75 \% indicates high heterogenity. Heterogenity should be viewed as a measure of inconsistency of the effect sizes of the underlying studies and not as a measure of the real variation between them.\\ 
...
\centering
\includegraphics[width = 0.7\textwidth]{appendix-forest}
...
\noindent
The forest plot visualizes the effect sizes and confidence intervals as well as the overall mean effect size and the weight assigned to each study in the meta-analysis. This can be helpful to spot studies with effect sizes far away from the rest, studies with large confidence intervals and studies that are the most influental to the pooled effect size. 
...
The significance of the effect size is one of the main concerns for a meta analysis. Therefore it is useful to repport whether left-out studies change the significance or not. \Sexpr{out1} \Sexpr{signif(rma.RE$pval, 3)}. \Sexpr{out2}.
